...
function setupMobileViewer() {
    ...
    // Show mobile viewer
    mobileViewer.style.display = 'flex';
    
    // Setup swipe gestures
    mobileContent.addEventListener('touchstart', handleTouchStart, { passive: true });
    mobileContent.addEventListener('touchmove', handleTouchMove, { passive: true });
    mobileContent.addEventListener('touchend', handleTouchEnd);
    
    // Tap to pause video
    mobileContent.addEventListener('click', handleTapToPause);
    
    // Mobile controls
    mobileSearchBtn.addEventListener('click', () => {
        const query = prompt('Search memes:');
        if (query) {
            filteredMemes = allMemes.filter(m => m.name.toLowerCase().includes(query.toLowerCase()));
    ...
    mobileLikeBtn.addEventListener('click', handleMobileLike);
}
...
function handleTapToPause(e) {
    const video = document.querySelector('.mobile-content video');
...
function nextMobileMeme() {
    if (currentMobileIndex < filteredMemes.length - 1) {
        currentMobileIndex++;
        renderMobileMeme();
    }
}

function previousMobileMeme() {
...
function renderMobileMeme() {
    ...
    const mobileStats = document.querySelector('.mobile-stats');
    const mobileLikeBtn = document.getElementById('mobileLikeBtn');
    const voteCountSpan = mobileLikeBtn.querySelector('.vote-count');
    ...
    mobileContent.appendChild(mediaElement);
    
    // Update info
    mobileName.textContent = meme.name;
    mobileStats.textContent = `${currentMobileIndex + 1} / ${filteredMemes.length}`;
    
    // Update like button
    const hasVoted = checkIfVoted(meme.filename);
    voteCountSpan.textContent = meme.votes || 0;
    if (hasVoted) {
    ...
    // Store current meme for like button
    mobileLikeBtn.dataset.filename = meme.filename;
}
...
async function handleMobileLike() {
    ...
    const hasVoted = checkIfVoted(filename);
    if (hasVoted) {
    ...
    if (success) {
        ...
        this.querySelector('.vote-count').textContent = meme.votes;
        
        // Heart animation
        const heart = document.createElement('div');
        heart.innerHTML = '❤️';
        heart.style.cssText = 'position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);font-size:80px;pointer-events:none;animation:heartPop 1s ease;z-index:10001;';
        document.body.appendChild(heart);
        setTimeout(() => heart.remove(), 1000);
    }
}

// Override loadMemes for mobile
const originalDisplayMemes = displayMemes;
displayMemes = function() {
    if (isMobile) {
        renderMobileMeme();
    } else {
        originalDisplayMemes();
    }
};
